...
class AutonomousDriver:
    ...
    def setup_inference_engines(self):
        # 1. Setup Semantic Segmentation Model (YOLO)
        # Assuming ONNX for YOLO as per the pipeline description
        rospy.loginfo("Loading YOLO Semantic Segmentation model...")
        self.yolo_session = YOLO(self.yolo_path, task="semantic")

        # 2. Setup PilotNet Model
        rospy.loginfo("Loading PilotNet model...")
        if USE_TENSORRT:
            self.logger = trt.Logger(trt.Logger.WARNING)
            with open(f"{self.pilotnet_path}.engine", "rb") as f, trt.Runtime(self.logger) as runtime:
    ...
    def image_cb(self, msg):
        try:
            ...
            cv_image = cv2.imdecode(np_arr, cv2.IMREAD_COLOR) # Shape: (480, 640, 3) BGR
            rgb_image = cv2.cvtColor(cv_image, cv2.COLOR_BGR2RGB)

            # 2. Prepare for YOLO Segmentation (1, 3, 480, 640)
